...
5. SET in-flight sentinel `session_inflight:{session_id}` with the same TTL (#759). Bracketed via `_InflightSentinel` async context manager so DEL fires on success **and** exception. Distinct from the resume lock; both now cover cold turns (the sentinel from inception, the lock since #779). Drives the `turn_in_progress` field on the GET endpoint.
6. Acquire `_ResumeLock(agent, cached_uuid, session.id, ttl_seconds=lock_ttl)` — Redis SET NX EX with async wait-and-retry (250 ms tick, 30 s ceiling). Two key shapes: warm turns use `_session_lock_key(agent, uuid)` → `session_lock:{agent}:{claude_session_id}`; cold turns key on `session_lock:cold:{session_id}` (#779) — same session serialised, different sessions still concurrent. Pre-#779 the cold path short-circuited to no-op and allowed two concurrent first-turn POSTs to race on `update_cached_claude_session_id` and orphan a JSONL.
...
| Synchronous socket severed mid-turn (intermediary read-timeout, e.g. Cloudflare ~100s) (#1376) | Frontend-only: `sendMessage`'s `catch` reconciles against `GET /sessions/{id}`. Order matters — **success is checked before the in-flight handoff**: a NEW assistant beyond the pre-send baseline → success first; only then `turn_in_progress=true` → hand off to the reattach poller (spinner stays, no error); neither → surface the real error. Success-first is load-bearing: the assistant row persists *inside* the inflight sentinel, so a reconcile GET can see assistant-present AND sentinel-still-set; handing off there would let `startPolling` rebaseline past the landed reply and false-fail. A **429** (concurrent-turn reject) short-circuits *before* the reconcile — `turn_in_progress` reflects the OTHER turn, so attaching would silently drop this message; the real concurrency error is surfaced instead. On a genuine pre-persistence drop the catch restores the optimistic user row (the reconcile GET's server truth lacks it) so the typed message stays visible with the error. The poller's terminal mirrors the catch: new assistant → success, sentinel cleared with no reply → "No reply came back…" error, wall-clock deadline → neutral "still checking" notice (`noticeBySession`, `role="status"`). `SessionPanel.vue` adds the UX polish: the error box gets `role="alert"`/`aria-live="assertive"` and the notice `role="status"`/`aria-live="polite"` so an async outcome landing minutes after the click is announced to screen readers, and the spinner copy flips to "Still working… this may take a while." after ~75s — kept alive across the poller handoff (not reset while the turn is still in flight) (D7) |
...
Playwright spec, marked `@interactive` (real Claude call ~10–60s, opt-in via `npm run test:e2e -- session-tab.spec`). Snapshots prior flag in `beforeAll`, restores in `afterAll`. Original three cases: tab hidden when off, tab appears + send turn + reset modal, Chat tab switch preserves Session state. **#1376** adds a `long-turn resilience` describe block (T1–T8) that stubs the turn POST (sever / 502 / 429) and `GET /sessions/{id}` with server-truth fixtures via `page.route` — no real Claude turn: severed-but-running (spinner, no error), severed-but-succeeded (reply renders), genuine 502 (real error), severed-then-failed ("No reply came back" error), pre-persistence false-success guard (baseline), deadline → neutral notice (driven by the `window.__TRINITY_SESSION_POLL_DEADLINE_MS__` test seam), **T7** assistant-present-AND-sentinel-still-set → success not a false "no reply" (the success-before-handoff ordering), and **T8** a 429 concurrent-turn reject → real error, never a silent reattach. Reaches the Session surface via `?tab=session` (unified Chat tab + session-mode toggle, #1112).
...
| **Long Session turns held a synchronous socket (mostly fixed, #1376)** | The Session turn endpoint is synchronous and may legitimately run for the agent's full execution timeout (up to 7200s). The Axios timeout matches at 7260s, but an intermediary with a shorter read timeout (notably an external **Cloudflare ~100s edge timeout** — operator config, not in this repo) can sever the socket mid-turn even though the work succeeds server-side. **#1376** made the frontend resilient: on any send error the store reconciles against authoritative server state (`turn_in_progress` + a pre-send assistant-count baseline) before declaring failure — a still-running turn hands off to the reattach poller (no error), an already-succeeded turn renders its reply, and a genuinely-failed-or-empty turn surfaces a real error. The poll uses a wall-clock deadline (~2h, covering the sentinel TTL); at the deadline it shows a neutral "still checking" notice (not a red error). | A long turn now keeps a spinner and lands its reply with no false "Failed to send". Remaining edge: if both the browser AND the reconcile GET are offline, the original error still shows (correct — server truth is unreachable). Async-mode + SSE (Epic #1045/#1083) would remove the held socket entirely. |
