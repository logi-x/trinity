    ...
    <div v-else class="flex h-[calc(100vh-220px)] overflow-hidden">
      ...
      <div class="w-80 min-w-[280px] max-w-[400px] border-r border-gray-200 dark:border-gray-700 flex flex-col">
        ...
        <div class="px-2 py-1.5 border-t border-gray-200 dark:border-gray-700 text-xs text-gray-500 dark:text-gray-400">
          <span>{{ totalFiles }} loaded files<span v-if="totalSize > 0"> &bull; {{ formatFileSize(totalSize) }}</span></span>
        </div>
      </div>
...
const loadFiles = async () => {
  ...
  try {
    const data = await agentsStore.listAgentFiles(props.agentName, '/home/developer', showHidden.value, 0)
...
const loadDirectory = async (item) => {
  ...
  if (!props.agentName || target.type !== 'directory' || target.children_loading || target.children_loaded) return
